	...
	switch(action)
	{
		case MenuAction_Select:
		{
			if(IsClientInGame(client))
			{
				...
				if(!CheckToSetWeapon(client, g_iIndex[client], skinId)) return 0;
	...
	if (IsPlayerAlive(client))
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if(weaponEntity != -1 && GetWeaponClass(weaponEntity, weaponClass, sizeof(weaponClass)))
			{
				...
				menu.AddItem(weaponClass, weaponName, (IsKnifeClass(weaponClass) && (g_iKnife[client] == 0 || g_iKnife[client] == 53)) ? ITEMDRAW_DISABLED : ITEMDRAW_DEFAULT);
				index++;
			}
		}
	}
