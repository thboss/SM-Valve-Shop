...
public void OnPluginStart()
{
	...
	g_Cvar_FloatIncrementSize 		= CreateConVar("sm_weapons_float_increment_size", 	"0.10", 			"Increase/Decrease by value for weapon float");
...
public Action CommandWeaponSkins(int client, int args)
{
	...
	{
		PrintToChat(client, "You don't have access to change skins before shop load, just wait few seconds and retry");
...
void SetWeaponProps(int client, int entity)
{
	...
	if (index > -1 && g_iSkins[client][index] != 0)
	{
		...
		SetEntProp(entity, Prop_Send, "m_nFallbackSeed", 8192);
		if(!IsKnife(entity))
		{
