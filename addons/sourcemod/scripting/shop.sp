...
public Plugin myinfo =
{
	...
	author = "FrozDark (Fork by R1KO & White Wolf & TheBO$$)",
...
public void OnPluginStart()
{
	...
	CreateConfigs();
	RegConsoleCmd("sm_inv", Commands_Inventory);
	RegConsoleCmd("sm_inventory", Commands_Inventory);		
		...
		case MenuAction_Select :
		{
			...
			switch (iButton[param1][param2])
			{
				...
				case BUTTON_TRANSFER :
				{
					...
					*/
						CreateVerificationCredits(param1);		
						CPrintToChat(param1, "%t", "type_price_or_cancle");					
					}
					else
					{
						ShowItemInfo(param1, iClItemId[param1]);
...
//type price in chat or !cancel\nCurrent: %d		
void CreateVerificationCredits(int param1)		
{		
	g_bTransfer[param1] = true;		
	Menu menu = new Menu(VerCredits_);		
	menu.SetTitle("%t", "verification_credits", g_iCreditsTransferAmount[param1]);		
	menu.AddItem(NULL_STRING, NULL_STRING, ITEMDRAW_SPACER);		
			
	char sBuffer[128];
	FormatEx(sBuffer, sizeof sBuffer, "%t", "confirm");		
	menu.AddItem(NULL_STRING, sBuffer, g_iCreditsTransferAmount[param1] > 2 ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);		
	...
	FormatEx(sBuffer, sizeof sBuffer, "%t", "gift_free");		
	menu.AddItem(NULL_STRING, sBuffer);
...
public int VerCredits_(Menu menu, MenuAction action, int param1, int param2)		
{		
	...
	{		
		if(param2 != 2)		
		{		
			if(param2 == 4) g_iCreditsTransferAmount[param1] = 0;		
			if (!SetupItemTransfer(param1))		
			{		
				CPrintToChat(param1, "%t", "no_targets");
				g_iCreditsTransferAmount[param1] = 0;
				ShowItemInfo(param1, iClItemId[param1]);
			}		
		}
		else if(param2 == 2)		
...
public int Menu_TransItemHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case MenuAction_Select :
		{
			...
			if(!g_iCreditsTransferAmount[param1]) 
			{
				ApplyGift(param1, target);
			}
			else
			{
				if(GetCredits(target) < g_iCreditsTransferAmount[param1])
				{
					SetupItemTransfer(param1);
					CPrintToChat(param1, "%t", "trade_not_enough_credits", target);
					return;
				}
				else VerificationTrade(param1);
			}	
...
void VerificationTrade(int client)		
{		
	...
	g_iItemTransTarget[target] = GetClientUserId(client);		
	Menu menu = new Menu(VerificationTrade_);		
	...
	ItemManager_GetCategoryDisplay(GetItemCategoryId(iClItemId[client]), client, category, sizeof(category));		
	// %N wants to give you %s from the category `%s` for %d credits		
	// Игрок %N хочет вам передать 		
	menu.SetTitle("%t", "VerificationTrade", client, item, category, g_iCreditsTransferAmount[client]);		
	char sBuffer[128];		
	FormatEx(sBuffer, sizeof sBuffer, "%t", "confirm");		
	menu.AddItem(NULL_STRING, sBuffer);		
	...
	FormatEx(sBuffer, sizeof sBuffer, "%t", "cancel");		
	menu.AddItem(NULL_STRING, sBuffer);		
	menu.Display(target, 30);		
}		
public int VerificationTrade_(Menu menu, MenuAction action, int param1, int param2)		
{		
	switch (action)		
	{		
		case MenuAction_Select :		
		{		
			int client = GetClientOfUserId(g_iItemTransTarget[param1]);		
			if (!client)		
			{		
				ShowItemInfo(param1, iClItemId[param1]);		
				CPrintToChat(param1, "%t", "target_left_game");		
			...
			if(!param2)		
			{		
				ApplyTrade(client, param1);		
						
				//ShowTransItemInfo(client);		
			}		
			else CPrintToChat(client, "%t", "trade_deny", param1 );		
		}		
		case MenuAction_Cancel:		
		{		
			int client = GetClientOfUserId(g_iItemTransTarget[param1]);		
			...
			{		
				ShowItemInfo(client, iClItemId[client]);		
				CPrintToChat(client, "%t", "transfer_failed");		
				return 0;		
			}		
		}		
...
void ApplyTrade(int client, int param1)		
{		
	if (!Forward_OnItemTransfer(client, param1, iClItemId[client]))		
	{		
		ShowItemInfo(client, iClItemId[client]);		
		return;		
	}		
			
	PlayerManager_TransferItem(client, param1, iClItemId[client]);		
			
	RemoveCredits(param1, g_iCreditsTransferAmount[client], CREDITS_BY_BUY_OR_SELL);		
	GiveCredits(client, g_iCreditsTransferAmount[client], CREDITS_BY_BUY_OR_SELL);		
			
	Forward_OnItemTransfered(client, param1, iClItemId[client]);		
			
	char category[SHOP_MAX_STRING_LENGTH], item[SHOP_MAX_STRING_LENGTH];		
	ItemManager_GetItemDisplay(iClItemId[client], param1, item, sizeof(item));		
	ItemManager_GetCategoryDisplay(GetItemCategoryId(iClItemId[client]), param1, category, sizeof(category));		
	CPrintToChat(param1, "%t", "receive_item", client, category, item);		
	CPrintToChat(client, "%t", "trade_apply", item, category, param1, g_iCreditsTransferAmount[client]);		
}

void ApplyGift(int client, int param1)		
{		
	if (!Forward_OnItemTransfer(client, param1, iClItemId[client]))		
	{		
		ShowItemInfo(client, iClItemId[client]);		
	...
	ItemManager_GetItemDisplay(iClItemId[client], param1, item, sizeof(item));		
	ItemManager_GetCategoryDisplay(GetItemCategoryId(iClItemId[client]), param1, category, sizeof(category));		
	CPrintToChat(param1, "%t", "receive_gift", client, item, category);		
	CPrintToChat(client, "%t", "gift_apply", item, category, param1);		
}
...
bool AddTargetsToMenu(Menu menu, int source_client, bool credits = true)
{
